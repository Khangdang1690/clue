...
{len(result.statistical_tests)} statistical tests were performed:
...
Each opportunity should be concrete and actionable.
...
"""
        ...
        for line in lines:
            line = line.strip()
            if line and (line.startswith('-') or line.startswith('•') or line[0].isdigit()):
                opportunities.append(line.lstrip('-•0123456789. '))
...
"""
        ...
        for line in lines:
            line = line.strip()
            if line and any(priority in line.lower() for priority in ['high', 'medium', 'low', 'critical', 'priority']):
                actions.append(line.lstrip('-•0123456789. '))
            elif line and (line.startswith('-') or line[0].isdigit()):
                actions.append(line.lstrip('-•0123456789. '))
