...
from src.discovery.data_cleaner import DataCleaner
...
class DiscoveryWorkflow:
    ...
    2. Clean data (8-step pipeline)
    3. Profile data (basic statistics)
    4. Autonomous exploration (LLM writes Python code)
    5. Generate business report

    This replaces the old fixed-analysis approach with adaptive exploration.
    """
    ...
    def __init__(
        ...
        self.data_cleaner = DataCleaner(**cleaning_config)

        # Data profiler
        self.profiler = DataProfiler(sample_size=100000)

        # Autonomous explorer (NEW - replaces fixed-analysis components)
    ...
    def _build_graph(self) -> StateGraph:
        """Build the new autonomous exploration workflow."""
        ...
        workflow.add_edge("generate_context", "clean_data")
        workflow.add_edge("clean_data", "profile_data")
    ...
    def _clean_data_node(self, state: DiscoveryState) -> DiscoveryState:
        """Node 2: Clean the dataset."""
        state["current_phase"] = "cleaning"

        try:
            if self.clean_data:
                print("\n[CLEANING] Cleaning data...")
                cleaned_df, cleaning_report = self.data_cleaner.clean(state["raw_data"])
                state["raw_data"] = cleaned_df
                state["cleaning_report"] = cleaning_report
                print(f"[OK] Data cleaned")
            else:
                print("\n[SKIP] Data cleaning skipped")
                state["cleaning_report"] = None

        except Exception as e:
            print(f"\n[WARN] Data cleaning failed: {e}")
    ...
    def _autonomous_explore_node(self, state: DiscoveryState) -> DiscoveryState:
        """Node 4: Autonomous exploration (NEW - replaces fixed analysis)."""
