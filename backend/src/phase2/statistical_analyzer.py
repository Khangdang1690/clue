...
class StatisticalAnalyzer:
    ...
    def analyze_challenge(
        ...
        # Discover relationships between datasets (for potential joins)
        if len(relevant_data) > 1:
            print(f"\n🔍 Discovering relationships between {len(relevant_data)} datasets...")
            relationships = self._discover_dataset_relationships(relevant_data, challenge, business_context)
            analysis_results["dataset_relationships"] = relationships
            if relationships:
                print(f"✓ Found {len(relationships)} potential dataset relationships")
...
Recommend analyses in JSON format (NO visualization recommendations - focus only on statistical analysis):
{{
...
"""
        ...
        try:
            ...
            if start_idx != -1 and end_idx > start_idx:
                result = json.loads(content[start_idx:end_idx])
                # Remove visualization recommendations if they exist
                result.pop('recommended_visualizations', None)
                return result
    ...
    def _check_thresholds_generic(
        ...
        Uses MEAN (average) for threshold comparison, not min/max extremes.
        This provides a realistic view of typical performance.
        ...
        print(f"\n📊 Checking {len(metric_targets)} business metric thresholds...")
        ...
        for metric_info in metric_targets:
            ...
            if matching_col and matching_col in df.select_dtypes(include=[np.number]).columns:
                ...
                col_std = df[matching_col].std()

                # Debug logging
                print(f"  📈 Metric: {matching_col}")
                print(f"     Min: {col_min:.4f} | Mean: {col_mean:.4f} | Max: {col_max:.4f} | Std: {col_std:.4f}")
                print(f"     Target: {target_value:.4f} (want {direction})")

                # Check if MEAN falls outside target (not min/max extremes)
                # This represents typical performance, not worst-case scenarios
                if direction == 'above' and col_mean < target_value:
                    gap = target_value - col_mean
                    if target_value < 1:  # Percentage format
                        alert_msg = (
                            f"CRITICAL: {matching_col} dropped to {col_mean:.1%}, "
                        ...
                        alert_msg = (
                            f"CRITICAL: {matching_col} dropped to {col_mean:.2f}, "
                            f"{gap:.2f} below target of {target_value:.2f}"
                        )
                        print(f"     ⚠️  ALERT: {alert_msg}")
                        alerts.append(alert_msg)
                elif direction == 'below' and col_mean > target_value:
                    gap = col_mean - target_value
                    if target_value < 1:  # Percentage format
                        alert_msg = (
                            f"CRITICAL: {matching_col} peaked at {col_mean:.1%}, "
                        ...
                        alert_msg = (
                            f"CRITICAL: {matching_col} peaked at {col_mean:.2f}, "
                ...
                print(f"  ⚠️  Metric '{metric_name}' matched to '{matching_col}' but it's not numeric - skipping")
            else:
                print(f"  ⚠️  No matching column found for metric '{metric_name}'")

        if alerts:
            print(f"\n⚠️  Generated {len(alerts)} threshold alerts")
        else:
...
"""
    ...
    def _discover_dataset_relationships(
        self,
        ...
        """
        Discover potential relationships between datasets for joining.

        Args:
            datasets: Dictionary of relevant DataFrames
            challenge: Challenge being analyzed
            business_context: Business context
        ...
        for i in range(len(dataset_list)):
            for j in range(i + 1, len(dataset_list)):
                name1, df1 = dataset_list[i]
                name2, df2 = dataset_list[j]

                # Try to find join keys using multiple strategies
                join_keys = self._find_join_keys_between_datasets(
                    df1, df2, name1, name2, challenge, business_context
                )
                ...
                if join_keys:
                    relationships.append({
                        'dataset1': name1,
                        'dataset2': name2,
                        'join_key1': join_keys[0],
                        'join_key2': join_keys[1],
                        'relationship_type': join_keys[2] if len(join_keys) > 2 else 'many-to-many',
                        'confidence': join_keys[3] if len(join_keys) > 3 else 'medium'
                    })

        return relationships

    def _find_join_keys_between_datasets(
        self,
        df1: pd.DataFrame,
        df2: pd.DataFrame,
        name1: str,
        ...
        """
        Find join keys between two datasets using multi-tier strategy.

        Returns:
            Tuple of (key1, key2, relationship_type, confidence) or None
        """
        # Strategy 1: Exact column name matches (highest confidence)
        common_cols = set(df1.columns).intersection(set(df2.columns))
        if common_cols:
            # Prioritize ID/key columns
            for col in common_cols:
                col_lower = col.lower()
                if any(keyword in col_lower for keyword in ['id', 'key', 'code', 'number']):
                    # Check cardinality to determine relationship type
                    card1 = len(df1[col].unique())
                    card2 = len(df2[col].unique())
                    rel_type = self._determine_relationship_type(card1, card2, len(df1), len(df2))
                    return (col, col, rel_type, 'high')

            # Use first common column if no ID column found
            first_col = list(common_cols)[0]
            return (first_col, first_col, 'many-to-many', 'medium')

        # Strategy 2: Similar column names
        for col1 in df1.columns:
            col1_lower = col1.lower().replace('_', '').replace('-', '').replace(' ', '')
            if not any(kw in col1_lower for kw in ['id', 'key', 'code']):
                continue

            for col2 in df2.columns:
                col2_lower = col2.lower().replace('_', '').replace('-', '').replace(' ', '')

                # Check if columns are similar
                if col1_lower in col2_lower or col2_lower in col1_lower:
                    # Verify data types are compatible
                    if self._are_columns_compatible(df1[col1], df2[col2]):
                        card1 = len(df1[col1].unique())
                        card2 = len(df2[col2].unique())
                        rel_type = self._determine_relationship_type(card1, card2, len(df1), len(df2))
                        return (col1, col2, rel_type, 'medium')

        # Strategy 3: Cross-reference detection (e.g., "customer_id" in orders → "id" in customers)
        # Check if dataset name appears in column names of the other dataset
        for col1 in df1.columns:
            col1_lower = col1.lower()
            name2_lower = name2.lower().replace('_', ' ').split()[0]  # First word of dataset name

            if name2_lower in col1_lower and any(kw in col1_lower for kw in ['id', 'key']):
                # Find matching column in df2
                for col2 in df2.columns:
                    col2_lower = col2.lower()
                    if any(kw in col2_lower for kw in ['id', 'key', 'pk', 'primary']):
                        if self._are_columns_compatible(df1[col1], df2[col2]):
                            return (col1, col2, 'many-to-one', 'medium')

        for col2 in df2.columns:
            col2_lower = col2.lower()
            name1_lower = name1.lower().replace('_', ' ').split()[0]

            if name1_lower in col2_lower and any(kw in col2_lower for kw in ['id', 'key']):
                # Find matching column in df1
                for col1 in df1.columns:
                    col1_lower = col1.lower()
                    if any(kw in col1_lower for kw in ['id', 'key', 'pk', 'primary']):
                        if self._are_columns_compatible(df1[col1], df2[col2]):
                            return (col1, col2, 'one-to-many', 'medium')

        # Strategy 4: LLM-based inference (for complex cases)
        return self._llm_infer_join_keys_with_context(
            df1, df2, name1, name2, challenge, business_context
        )
    ...
    def _are_columns_compatible(self, col1: pd.Series, col2: pd.Series) -> bool:
        ...
        if col1.dtype != col2.dtype:
            # Try to see if they're both numeric or both string-like
            numeric_types = ['int64', 'int32', 'float64', 'float32']
            if not ((col1.dtype in numeric_types and col2.dtype in numeric_types) or
                    (col1.dtype == 'object' and col2.dtype == 'object')):
                return False

        # Check if there's any overlap in values (sample check)
        sample1 = set(col1.dropna().head(100).astype(str))
        sample2 = set(col2.dropna().head(100).astype(str))
        overlap = len(sample1.intersection(sample2))
    ...
    def _determine_relationship_type(
        ...
        # One-to-one: both columns have unique values close to row count
        if unique1 >= rows1 * 0.9 and unique2 >= rows2 * 0.9:
            return 'one-to-one'

        # One-to-many: one side is mostly unique, other is not
        if unique1 >= rows1 * 0.9 and unique2 < rows2 * 0.5:
            return 'one-to-many'

        # Many-to-one: opposite of above
        if unique2 >= rows2 * 0.9 and unique1 < rows1 * 0.5:
            return 'many-to-one'

        # Default: many-to-many
        return 'many-to-many'

    def _llm_infer_join_keys_with_context(
        self,
        df1: pd.DataFrame,
        df2: pd.DataFrame,
        name1: str,
        ...
        """Use LLM to infer join keys with business context."""
        inference_prompt = ChatPromptTemplate.from_template(
            """Identify potential join keys between these datasets for business analysis.

Challenge: {challenge}
Business Context: {business_goal}
...
Based on the business context and challenge, which columns should be used to join these datasets?
Consider:
- Common business entities (customers, employees, products, etc.)
- Foreign key relationships
- Data types and sample values

Respond with JSON:
{{
    "join_key_1": "column_in_dataset1" or null,
    "join_key_2": "column_in_dataset2" or null,
    "relationship_type": "one-to-one|one-to-many|many-to-one|many-to-many",
    "confidence": "high|medium|low",
    "business_reasoning": "why these datasets should be joined"
}}

If no suitable join exists, set join keys to null.
"""
        )
        ...
        try:
            ...
            if start_idx != -1 and end_idx > start_idx:
                ...
                rel_type = result.get('relationship_type', 'many-to-many')
                confidence = result.get('confidence', 'low')

                if key1 and key2 and key1 in df1.columns and key2 in df2.columns:
                    print(f"  🤖 I suggested join: {name1}.{key1} = {name2}.{key2} ({rel_type}, confidence: {confidence})")
                    return (key1, key2, rel_type, confidence)

        except Exception as e:
            print(f"  ⚠ LLM join inference failed: {e}")
